...
class YoloDet:
    def __init__(self, class_id: int, cx: float, cy: float, w: float, h: float, conf: float = 1.0):
...
def _load_dataset(gt_dir: str, pred_dir: str) -> Tuple[Dict[str, List[YoloDet]], Dict[str, List[YoloDet]]]:
...
def _evaluate_class(class_id: int,
...
def map_range(iou_start: float = 0.5, iou_end: float = 0.95, iou_step: float = 0.05) -> Tuple[float, Dict[int, float]]:
...
def main():
    ...
    print(f"推理目录: C:\\Users\\lqx94\\Desktop\\raceone\\test\\inferresult\\inferlabels")
    print("=" * 60)
