...
def write_txt_tables(counts_with_tables, rid, team, num=None):
    """
    写入多表合并结果文件。counts_with_tables: List[Tuple[counts_dict, table_int]]
    结果格式：
    START\n
    Goal_ID=...;Num=...;Table=1\n
    Goal_ID=...;Num=...;Table=2\n
    ...
    os.makedirs(RESULT_FOLDER, exist_ok=True)
    lines = ["START"]
    for counts, table in counts_with_tables:
        table_int = int(table)
        for k, v in (counts or {}).items():
            if k == "desk":
                continue
            try:
                lines.append(f"Goal_ID={k};Num={int(v)};Table={table_int}")
            except Exception:
                # 忽略不可转 int 的值
                pass
        # 兼容历史分支：需要附加数学题条目
        if num == "math":
            lines.append(f"Goal_ID=W001;Num=1;Table={table_int}")
            lines.append(f"Goal_ID=W002;Num=1;Table={table_int}")
    lines.append("END")
    path = os.path.join(RESULT_FOLDER, f"{team}-R{rid}.txt")
