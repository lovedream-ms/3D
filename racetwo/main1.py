"""PyQt6 GUI for camera streaming, image capture, YOLO detection, and judge box integration.

本模块提供一个简洁的图形界面：
- 打开/关闭摄像头、实时显示视频帧
- 抓拍单帧保存到 data/ 目录
- 第二轮：三次检测（Table=1,2,3），中间发送旋转并等待，必要时向裁判箱发送开始/结果和 OCR 统计
- 支持命令行参数方式自动运行（python main1.py 1）
"""
import os, sys
import warnings
import time
...
class MainWindow(QMainWindow):
    ...
    def start2Button(self, checked=False):
        ...
        try:
            ...
            # Table=3：使用投票策略 - 拍摄18张，使用最后15张推理
            self.status_label.setText("Status: Camera Round 2 - Table 3")
            self.status_label.setStyleSheet("color: orange; padding: 4px;")
            self.log("Table 3: Capturing 18 images with 0.4s interval...")
            
            # 拍摄18张图片，间隔0.4秒
            bgr_session, bgr_paths = self._captured_multiple_images_session(num_images=18, interval=0.4)
            
            # 转换BGR到RGB - 只转换最后15张
            if len(bgr_paths) >= 15:
                bgr_paths_to_convert = bgr_paths[-15:]  # 取最后15张
                self.log(f"Converting last 15 images from {len(bgr_paths)} total captured...")
            else:
                bgr_paths_to_convert = bgr_paths
                self.log(f"Converting all {len(bgr_paths)} images (less than 15 captured)...")
            ...
            # 使用投票策略进行结果统计
            self.log(f"Using voting strategy for {len(per_image_counts)} images...")
    ...
    def _table3_voting_strategy(self, per_image_counts):
        """
        Table 3 专用投票策略：
        - 类别存在条件：在15张图片中至少出现5次
        - 数量判定：如果数量2出现超过4次，则为2；否则为1
        """
        from collections import defaultdict
        ...
        for class_name, count_list in class_occurrences.items():
            # 类别至少出现5次才认为存在
            if len(count_list) < 5:
                self.log(f"  {class_name}: appeared {len(count_list)} times (< 5), ignored")
                continue
            
            # 统计数量2出现的次数
            count_2_occurrences = sum(1 for c in count_list if c == 2)
            
            # 如果数量2出现超过4次，则认为数量是2，否则是1
            if count_2_occurrences > 4:
                final_counts[class_name] = 2
                self.log(f"  {class_name}: appeared {len(count_list)} times, count=2 appeared {count_2_occurrences} times -> final: 2")
            else:
                final_counts[class_name] = 1
