...
def Merge_box(
    ...
    if (
        boxes1_all is None
        or confs1_all is None
        or clses1_all is None
        or overall_detector is None
    ):
        ...
        if boxes_xyxy is not None and boxes_xyxy.size > 0:
            for (x1, y1, x2, y2), score, cid in zip(
                boxes_xyxy.tolist(), confs2.tolist(), clses2.tolist()
            ):
                cname = (
                    local_detector.classes[cid]
                    if 0 <= cid < len(local_detector.classes)
                    else str(cid)
                )
        ...
        desk_boxes = None
        desk_ids = [
            i for i, name in enumerate(local_detector.classes) if name == "desk"
        ]
    ...
    try:
        ...
        non_desk_mask_m1 = np.logical_not(
            np.isin(
                clses1_all,
                np.array(
                    [
                        i
                        for i, name in enumerate(overall_detector.classes)
                        if name == "desk"
                    ],
                    dtype=np.int32,
                ),
        ...
        if boxes1_filtered is not None and len(boxes1_filtered) > 0:
            ...
            for (bx1, by1, bx2, by2), bconf, bcid in zip(
                boxes1_filtered.tolist(),
                confs1_filtered.tolist(),
                clses1_filtered.tolist(),
            ):
                bname = (
                    overall_detector.classes[bcid]
                    if 0 <= int(bcid) < len(overall_detector.classes)
                    else str(bcid)
                )
                if bname == "desk":
                    continue
                thr1 = float(
                    M1_CLASS_SCORE_THRESHOLDS.get(
                        bname, M2_CLASS_SCORE_THRESHOLDS.get(bname, PRED_CONF_THRES)
                    )
                )
                if float(bconf) < thr1:
                    continue
        ...
        if len(m1_boxes_in_crop) > 0:
            for m1_box, m1_conf, m1_cid in zip(
                m1_boxes_in_crop, m1_confs_in_crop, m1_clses_in_crop
            ):
                best_iou = 0.0
                best_j = -1
                ...
                if best_iou >= float(CROSS_MODEL_MATCH_IOU) and best_j >= 0:
                    ...
                    pref = str(
                        MERGE_CLASS_PREFERENCE.get(m1_name)
                        or MERGE_CLASS_PREFERENCE.get(m2_name)
                        or ""
                    )
                ...
                # 检查补框策略
                supplement_allowed = str(
                    SUPPLEMENT_ALLOWED_SOURCES.get(m1_name_unmatched, "both")
                )
                if supplement_allowed.lower() in ("overall", "both"):
                    boxes2_list.append(
                        [int(m1_box[0]), int(m1_box[1]), int(m1_box[2]), int(m1_box[3])]
                    )
        ...
        if len(boxes2_list) > 0:
            ...
            for j in range(len(boxes2_list)):
                ...
                supplement_allowed = str(
                    SUPPLEMENT_ALLOWED_SOURCES.get(cname_j, "both")
                )
    ...
    if boxes_xyxy is not None and boxes_xyxy.size > 0:
        for (x1, y1, x2, y2), score, cid in zip(
            ...
            color = box_random_color(cname)
            cv2.rectangle(
                stage3_img, (int(x1), int(y1)), (int(x2), int(y2)), color, BOX_THICKNESS
            )
            label = f"{cname}:{score:.2f}"
            cv2.putText(
                stage3_img,
...
def Overall_Address(frame, overall_detector):
    ...
    try:
        if boxes_xyxy is not None and len(boxes_xyxy) > 0:
            for (x1, y1, x2, y2), score, cid in zip(
                boxes_xyxy.tolist(), confs.tolist(), clses.tolist()
            ):
                cname = (
                    overall_detector.classes[cid]
                    if 0 <= cid < len(overall_detector.classes)
                    else str(cid)
                )
                thr = float(
                    M1_CLASS_SCORE_THRESHOLDS.get(
                        cname, M2_CLASS_SCORE_THRESHOLDS.get(cname, PRED_CONF_THRES)
                    )
                )
                if float(score) < thr:
                    continue
...
def Local_Address(
    ...
    if boxes2 is not None and boxes2.size > 0:
        ...
        # 在原图上绘制模型2的原始检测结果
        for (x1, y1, x2, y2), score, cid in zip(
            local_boxes_xyxy.tolist(), confs2.tolist(), clses2.tolist()
        ):
            cname = (
                local_detector.classes[cid]
                if 0 <= cid < len(local_detector.classes)
            ...
            color = box_random_color(cname)
            cv2.rectangle(
                local_img, (int(x1), int(y1)), (int(x2), int(y2)), color, BOX_THICKNESS
            )
            label = f"{cname}:{score:.2f}"
            cv2.putText(
                local_img,
        ...
        try:
            ...
            for i, (cid, sc) in enumerate(zip(clses2.tolist(), confs2.tolist())):
                ...
                if cname == "desk":
                    continue
                with open(
                    os.path.join(perobj_save_dir, f"obj_{i:03d}.txt"),
                    "w",
                    encoding="utf-8",
                ) as f:
...
def detect_round1(
    ...
    if overall_detector is not None and local_detector is not None:
        # 1. 模型1：全图检测
        stage1_img, boxes1, confs1, clses1, desk_boxes = Overall_Address(
            frame, overall_detector
        )
        ...
        if session_dir is not None:
            try:
                ...
                if boxes1 is not None and len(boxes1) > 0:
                    for i, (cid, sc) in enumerate(
                        zip(clses1.tolist(), confs1.tolist())
                    ):
                        name = (
                            overall_detector.classes[cid]
                            if 0 <= cid < len(overall_detector.classes)
                        ...
                        if name == "desk":
                            continue
                        with open(
                            os.path.join(session_dir, f"obj_{i:03d}.txt"),
                            "w",
                            encoding="utf-8",
                        ) as f:
        ...
        if desk_boxes is not None:
            # 2. 桌子裁剪和扩展
            crop_img, crop_xyxy = Cut_Desk_Extend(
                frame, desk_boxes, expansion_ratio=DESK_EXPANSION_RATIO
            )
            ...
            if crop_img is not None:
                ...
                out_img = (
                    ...
                    else (local_img if local_img is not None else stage3_img)
                )
...
def _write_txt(counts, rid, team, table: int = 1, num=None):
    ...
    def _lines_for(counts_map, table_id: int):
        return (
            ["START"]
            + [
                f"Goal_ID={k};Num={int(v)};Table={int(table_id)}"
    ...
    if num == "math":
        ...
        lines = (
            lines[:-1]
            + [
                "Goal_ID=W001;Num=1;Table={}".format(int(table)),
                "Goal_ID=W002;Num=1;Table={}".format(int(table)),
            ]
            + ["END"]
        )
        path = os.path.join(RESULT_FOLDER, f"{team}-R{rid}.txt")
